...
Eval compute in head (Cons Monday (Cons Tuesday Nil)) Sunday.
...
Lemma test1 : forall w1 w2 : week_day, w1 = w2 -> w1 = w2.
Proof.
  intros.
  apply H.
Qed.
...
Proof.
  intros.
  destruct H; [right; apply H | left; apply H].
Qed.
...
Proof.
  intros x y.
  case x; case y; compute; (reflexivity || intros H0; inversion H0).
Qed.
...
Proof.
  exists Saturday.
  intro.
  assert (is_a_member Saturday work_week = false).
  compute; reflexivity.
  assert (is_a_member Saturday work_week = true).
